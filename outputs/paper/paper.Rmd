---
title: "Narrow Victory for Biden Projected in 2020 US Presidential Election."
...
**Keywords:** US 2020 Election; forecasting; multi-level regression with post-stratification; Biden; Trump "
toc: no
---
...
survey_data %>%
  ggplot(aes(x = race, fill = vote_2020)) + 
  geom_bar(position = "dodge") + 
  coord_flip() +   
  theme_minimal() +
  scale_fill_brewer(palette = "Set2") +
...
survey_data %>%
  ggplot(aes(x = educd, fill = vote_2020)) + 
  geom_bar(position = "dodge") + 
  coord_flip() +   
  theme_minimal() +
  scale_fill_brewer(palette = "Set2") +
...
survey_data %>%
  ggplot(aes(x = age, fill = vote_2020)) + 
  geom_bar(position = "dodge") + 
  coord_flip() +   
  theme_minimal() +
  scale_fill_brewer(palette = "Set2") +
...
survey_data %>%
  ggplot(aes(x = state, fill = vote_2020)) + 
  geom_bar(position = "dodge") + 
  coord_flip() +   
  theme_minimal() +
  scale_fill_brewer(palette = "Set2") +
...
```{r educd, echo=FALSE, message = FALSE,warning = FALSE , fig.height = 3, fig.width = 3.5, fig.cap = "Distribution of Education"}


educate <- aggregate(x=census_data$Number, by = list(census_data$educd), FUN = sum)
...
post_strat_est_edu %>%
ggplot(aes(y = mean, x = forcats::fct_inorder(educd), color = "MRP estimate")) + geom_point() +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0) + ylab("Proportion Biden Support") + xlab("Education") + geom_point(data = survey_data %>% 
                                                                                           group_by(educd, dummy_vote) %>%
                                                                                           summarise(n = n()) %>%
                                                                                           group_by(educd) %>%
                                                                                           mutate(prop = n/sum(n)) %>%
                                                                                           filter(dummy_vote==1),
                                                                                         aes(educd, prop, color = "Raw Data"))+
